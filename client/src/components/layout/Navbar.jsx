          ...
          <div className="hidden sm:flex sm:items-center sm:space-x-8">
            {isAuthenticated ? (
              <>
                <Link
                  to="/"
                  className="text-slate-600 hover:text-brand-600 font-medium px-3 py-2 rounded-md transition-colors"
                >
                  Dashboard
                </Link>
                <Link
                ...
                {user?.role === "admin" && (
                  <Link
                    to="/admin"
                    className="text-brand-600 hover:text-brand-700 font-bold px-3 py-2 rounded-md transition-colors bg-brand-50 border border-brand-100 shadow-sm"
                  >
                    Admin Portal
                  </Link>
                )}
                <div className="flex items-center gap-4 ml-4">
                  ...
                  <Link
                    to="/bookings"
                    className="text-sm font-semibold text-brand-600 hover:text-brand-700 transition-colors bg-brand-50 hover:bg-brand-100 px-3 py-1.5 rounded-full"
                  >
                    My Bookings
                  </Link>
                  <Link
              ...
              <div className="flex items-center space-x-4">
                ...
                <Link
                  to="/register"
                  className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-lg font-medium shadow-sm transition-colors blur-0"
                >
                  Get Started
                </Link>
              </div>
                ...
                {isAuthenticated ? (
                  <>
                    ...
                    <div className="px-3 space-y-1">
                      <Link
                        to="/"
                        className="block px-4 py-3 text-base font-medium text-slate-700 hover:text-brand-600 hover:bg-brand-50 rounded-xl transition-colors min-h-[44px]"
                        onClick={() => setIsMobileMenuOpen(false)}
                      >
                        Dashboard
                      </Link>
                      ...
                      {user?.role === "admin" && (
                        <>
                          ...
                          <Link
                            to="/admin"
                            className="block px-4 py-3 text-base font-bold text-brand-700 bg-brand-50 hover:bg-brand-100 rounded-xl min-h-[44px]"
                            onClick={() => setIsMobileMenuOpen(false)}
                          >
                            Admin Portal
                          </Link>
                  ...
                  <div className="px-6 py-4 flex flex-col gap-4">
                    <Link
                      to="/login"
                      className="block text-center w-full px-4 py-3 border border-slate-300 rounded-xl shadow-sm text-base font-bold text-slate-700 bg-white hover:bg-slate-50 min-h-[44px] flex items-center justify-center"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      Sign in
                    </Link>
                    <Link
                      to="/register"
                      className="block text-center w-full px-4 py-3 border border-transparent rounded-xl shadow-sm text-base font-bold text-white bg-brand-600 hover:bg-brand-700 min-h-[44px] flex items-center justify-center"
                      onClick={() => setIsMobileMenuOpen(false)}
                    >
                      Get Started
                    </Link>

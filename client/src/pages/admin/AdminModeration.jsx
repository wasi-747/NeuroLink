...
import { Loader2, ShieldAlert, CheckCircle, AlertTriangle, Trash2, ShieldCheck, Flag } from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 font-medium mt-1">Review flagged community forum posts and comments.</p>
      </div>
      ...
      {reports.length === 0 ? (
        <div className="bg-white rounded-3xl border-2 border-dashed border-slate-200 p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 font-medium">There are no pending reports to review. Great job!</p>
        </div>
      ) : (
        <div className="space-y-4">
          {reports.map((report) => (
            <div key={report._id} className="bg-white rounded-2xl border border-red-100 shadow-sm overflow-hidden flex flex-col md:flex-row">
              {/* Left sidebar info */}
              <div className="bg-red-50/50 p-6 md:w-64 border-b md:border-b-0 md:border-r border-red-100 shrink-0">
                <div className="flex items-center gap-2 mb-4">
                  <Flag className="w-4 h-4 text-red-500" />
                  <span className="text-xs font-black uppercase tracking-wider text-red-700">Flagged {report.contentType}</span>
                </div>
                
                <div className="space-y-3 text-sm">
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase">Reason given</div>
                    <div className="font-semibold text-slate-800 mt-0.5">{report.reason}</div>
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase">Reported By</div>
                    <div className="font-semibold text-slate-800 mt-0.5">{report.reportedBy?.anonymousAlias || "Unknown"}</div>
                  </div>
                  <div>
                    <div className="text-xs font-bold text-slate-400 uppercase">Date</div>
                    <div className="font-semibold text-slate-800 mt-0.5">{format(parseISO(report.createdAt), "MMM d, yyyy h:mm a")}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <div className="mb-2 text-xs font-bold text-slate-400 uppercase tracking-wider">Reported Content Preview</div>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-6 relative">
                  {report.contentId ? (
                    <>
                      {report.contentType === 'post' && (
                        <h4 className="font-extrabold text-slate-900 mb-2">{report.contentId.title}</h4>
                      )}
                      <p className="text-slate-700 italic font-medium whitespace-pre-wrap">"{report.contentId.content}"</p>
                    </>
                  ) : (
                    <p className="text-slate-400 italic">Content has already been deleted by the user.</p>
                  )}
                </div>
